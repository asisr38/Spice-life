...
export default function ContactPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-accent/30">
              ...
              <Card key={index} className="card-elevated">
                <CardContent className="p-6">
                  <div className="flex items-center space-x-4 mb-4">
                    ...
                    <Heading as="h3" className="text-lg">{info.title}</Heading>
                  </div>
                  <Text className="mb-4">{info.description}</Text>
                  <ul className="space-y-2">
                    {info.details.map((detail, idx) => (
                      <li key={idx}>
                        <Text className="font-medium text-foreground">{detail}</Text>
                      </li>
                    ))}
                  </ul>
                </CardContent>
      ...
      <MotionSection className="section-padding bg-muted">
        <div className="max-w-4xl mx-auto container-padding text-center">
          <iframe
            ...
            allowFullScreen
            loading="lazy"
            className="rounded-md shadow-md w-full max-w-2xl mx-auto"
          ></iframe>
